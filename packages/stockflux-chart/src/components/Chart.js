...
const Chart = ({ chartData }) => {
  ...
  function makeChart(data) {
    ...
    var x2Axis = d3.axisTop(x2Scale).ticks(d3.timeYear.every(1)).tickFormat(d3.timeFormat("%Y"))
    ...
    var navigationMulti = fc.seriesSvgMulti()
      .series([area, brush]).decorate(function (selection) {
    ...
    chartData.currentValueCrosshair = chartData.series[chartData.series.length - 1]
    ...
    var navigatorChart = fc.chartCartesian(xScale.copy(), yScale.copy())
      .decorate(selection => {
        selection.enter().append('d3fc-svg').style('grid-column', 3)
          .style('grid-row', 2).style("height", '1em').classed('secondary-x-axis', true).on('measure.x2-axis', (d, i, nodes) => {
      ...
      .svgPlotArea(navigationMulti).xTicks(d3.timeMonth.every(2)).xTickFormat(d3.timeFormat("%B"))
