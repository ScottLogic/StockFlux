...
export default function() {
  ...
  function renderInternal() {
    ...
    containers.overlay.datum(model.overlay).call(overlay);
  ...
  function initialiseNavReset() {
    return menu.navigationReset().on(event.resetToLatest, resetToLatest);
  }
        ...
        if (externalHistoricFeedErrorCallback) {
          ...
          try {
            var responseObject = err.responseText
              ...
              : err;
            var formattedMessage = source.historicNotificationFormatter(
              responseObject
            );
            if (formattedMessage) {
  ...
  function updatePrimaryChartIndicators() {
    model.charts.primary.indicators = model.selectors.indicatorSelector.options.filter(
      function(option) {
  ...
  function updateSecondaryChartModels() {
    model.charts.secondary.indicators = model.selectors.indicatorSelector.options.filter(
      function(option) {
        return option.isSelected && !option.isPrimary;
      }
    );

    charts.secondaries().charts(
      model.charts.secondary.indicators.map(function(indicator) {
        return indicator;
      })
    );
  ...
  function onNotificationClose(id) {
    model.notificationMessages.messages = model.notificationMessages.messages.filter(
      function(message) {
  ...
  function addGdaxProducts(error, bitcoinProducts) {
    if (error) {
      ...
      var formattedProducts = formatGdaxProducts(
        bitcoinProducts,
        model.sources.bitcoin,
        defaultPeriods,
        productPeriodOverrides
      );
      model.headMenu.products = model.headMenu.products.concat(
        formattedProducts
      );
  ...
  app.run = function(element) {
    ...
    initialiseResize();
    _dataInterface(
      model.headMenu.selectedPeriod.seconds,
      model.headMenu.selectedProduct
    );
