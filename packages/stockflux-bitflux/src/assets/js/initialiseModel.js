...
export default function() {
  function initialisePeriods() {
    return {
      week1: model.data.period(
        'Weekly',
        60 * 60 * 24 * 7,
        { unit: d3.time.week, value: 1 },
        '%b %d'
      ),
      day1: model.data.period(
        'Daily',
        60 * 60 * 24,
        { unit: d3.time.day, value: 1 },
        '%b %d'
      ),
      hour1: model.data.period(
        '1 Hr',
        60 * 60,
        { unit: d3.time.hour, value: 1 },
        '%b %d %Hh'
      ),
      minute5: model.data.period(
        '5 Min',
        60 * 5,
        { unit: d3.time.minute, value: 5 },
        '%H:%M'
      ),
      minute1: model.data.period(
        '1 Min',
        60,
        { unit: d3.time.minute, value: 1 },
        '%H:%M'
      )
    };
  }
  ...
  function initialiseIndicatorOptions() {
    ...
    var indicators = [
      ...
      model.menu.option(
        ...
        'rsi',
        rsi,
        'bf-icon-rsi-indicator',
        false
      ),
      model.menu.option('MACD', 'macd', macd, 'bf-icon-macd-indicator', false),
      model.menu.option('Volume', 'volume', volume, 'bf-icon-bar-series', false)
    ];
  ...
  function initialiseCharts() {
    ...
    var primary = model.chart.primary(
      products.generated,
      products.generated.source.discontinuityProvider
    );
    var secondary = model.chart.secondary(
      products.generated,
      products.generated.source.discontinuityProvider
    );
    var xAxis = model.chart.xAxis(
      periods.day1,
      products.generated.source.discontinuityProvider
    );
  ...
  return {
    ...
    headMenu: model.menu.head(
      [products.stockFlux, products.generated, products.quandl],
      products.generated,
      periods.day1
    ),
