...
var intervals = [
  ...
  return createInterval.apply(this, interval);
});
...
export default function() {
  ...
  function tickIntervalForMultiYearInterval(mappedDomain, m) {
    var extent = scaleExtent(
        mappedDomain.map(function(d) {
          return d / year;
        })
      ),
  ...
  function minimumTickIntervalIndex() {
    ...
    if (minimumTickInterval != null && minimumTickInterval.length === 2) {
      intervals.forEach(function(interval, intervalIndex) {
        if (
          interval.interval === minimumTickInterval[0] &&
          interval.step === minimumTickInterval[1]
        ) {
          i = intervalIndex;
        }
      });
  ...
  function tickInterval(extent, count) {
    ...
    if (i === intervals.length) {
      ...
      var intervalIndex =
        i > 0 &&
        target / intervals[i - 1].duration < intervals[i].duration / target
          ? i - 1
          : i;
      while (
        span / intervals[intervalIndex].duration > count &&
  ...
  function tickValues() {
    ...
    var calculatedTicks = interval.range(
      extent[0],
      new Date(+extent[1] + 1),
      Math.max(1, step)
    );
