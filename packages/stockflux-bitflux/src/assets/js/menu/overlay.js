...
export default function() {
  ...
  var primaryChartIndicatorToggle = editIndicatorGroup().on(
    event.indicatorChange,
    dispatch[event.primaryChartIndicatorChange]
  );

  var secondaryChartToggle = editIndicatorGroup().on(
    event.indicatorChange,
    dispatch[event.secondaryChartChange]
  );

  var dataProductDropdown = dropdown().on(
    'optionChange',
    dispatch[event.dataProductChange]
  );

  var secondariesDataJoin = fc.util
    .dataJoin()
  ...
  var overlay = function(selection) {
    selection.each(function(model) {
      ...
      container
        .select('#mobile-product-dropdown')
        .datum({
          ...
          selectedIndex: products
            ...
            .indexOf(model.selectedProduct.id)
        })
        .call(dataProductDropdown);

      container
        .select('#overlay-primary-container .edit-indicator-container')
        .datum({ selectedIndicators: model.primaryIndicators })
        .call(primaryChartIndicatorToggle);

      var secondariesContainer = container.select(
        '#overlay-secondaries-container'
      );

      var secondaries = secondariesDataJoin(
        secondariesContainer,
        model.secondaryIndicators
      );

      var editIndicatorContainer = secondaries
        .enter()
        .append('div')
