...
export default function(containers, charts) {
  ...
  containers.overlaySecondaries.style(
    'flex',
    charts.secondaries().charts().length
  );

  var headRowHeight = parseInt(
    containers.app.select('.head-row').style('height'),
    10
  );
  if (!renderedOnce) {
    headRowHeight +=
  ...
  var useableHeight =
    fc.util.innerDimensions(containers.app.node()).height - headRowHeight;
