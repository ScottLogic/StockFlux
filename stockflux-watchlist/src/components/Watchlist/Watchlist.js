	...
	const onDragOver = e => {
		if (dragStartClientY) {
			const dragOverIndexOffset = Math.ceil(
				((e.nativeEvent.clientY - dragStartClientY) / (cardHeight / 2) +
					1) /
					2
			);
			...
			if (
				watchlist[nextDragOverIndex] &&
				nextDragOverIndex !== dragOverIndex
			) {
				...
				<div className="card-container">
					{watchlist.length === 0 && (
						<div className="no-watchlist">
							...
							<p>
								Use the search tab to add new stocks to the
								list.
							</p>
						</div>
					)}
